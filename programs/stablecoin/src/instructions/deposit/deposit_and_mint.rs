use crate::{
    ...
    state::Collateral,
    Config, SEED_COLLATERAL_ACCOUNT, SEED_CONFIG_ACCOUNT, SEED_SOL_ACCOUNT,
};
use anchor_lang::prelude::*;
use anchor_spl::{
    associated_token::AssociatedToken, token_interface::Mint, token_interface::Token2022,
...
pub fn deposit_collateral_and_mint_tokens(
    ...
    collateral_account.amount_minted += mint_amount;

    //   e If not initialized, it will be initialized....
    if !collateral_account.is_initialized {
        collateral_account.is_initialized = true;
        collateral_account.depositer = ctx.accounts.depositor.key();
    ...
    // e depositing sol to the sol account
    deposit_sol(
        &ctx.accounts.depositor,
        &ctx.accounts.sol_account,
        &ctx.accounts.system_program,
        mint_amount,
    )?; //e Gotta unwrap/error check it..

    //e The bump needed was the bump for the mint account
    mint_tokens(
