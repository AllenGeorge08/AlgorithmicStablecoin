use crate::{
    constants::{FEED_ID, PRICE_FEED_DECIMAL_ADJUSTMENT},
    errors::CustomError,
    Collateral, Config, MAXIUMUM_AGE,
};
...
pub fn calculate_health_factor(
    ...
    let health_factor =
        (collateral_adjusted_for_liquidation_threshold) / (collateral.amount_minted);

    Ok(health_factor)
}

pub fn get_usd_value(amount_in_lamports: &u64, price_feed: &Account<PriceUpdateV2>) -> Result<u64> {
    let feed_id = get_feed_id_from_hex(FEED_ID)?; //@audit this q mark is very imp..Haha!
    let price = price_feed.get_price_no_older_than(&Clock::get()?, MAXIUMUM_AGE, &feed_id)?;

    //e Require macro,cool!
    require!(price.price != 0, CustomError::InvalidPrice);

    let price_in_usd = price.price as u128; //e We got the price in usd
...
pub fn get_lamports_from_usd(
    ...
    require!(price.price > 0, CustomError::InvalidPrice);
    let price_in_usd = price.price as u128 * PRICE_FEED_DECIMAL_ADJUSTMENT; //e Scaling the decimals
    let account_in_lamports = (*amount_in_usd as u128 * (LAMPORTS_PER_SOL as u128)) / price_in_usd;
    Ok(account_in_lamports as u64) //q Wouldn't downcasting it lead to fund loss?
}
